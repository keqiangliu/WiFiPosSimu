...
legend('\sigma  = 1','\sigma  = 5','\sigma  = 8','\sigma  = 10','\sigma  = 15','location','southeast');
...
linecolors = jet(100);
figure('pos',[600 100 930 800]);
for i= 1:5
    j1=(i-1)*100+1;j2=i*100;
    
    % plot nn cdf
    subplot(5,2,2*i-1);    
    temp=sortrows(result(j1:j2,:),2);
    nn_error=temp(:,4);
...
temp=cell2mat(result(:,2));
xmin=roundn(min(temp),-2);xmax=roundn(max(temp),-2); %??
h=colorbar('SouthOutside');caxis([xmin xmax]);
set(get(h,'title'),'string','CV','fontname','Times New Roman','fontsize',13,'fontweight','bold');
